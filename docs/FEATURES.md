...
| M6 | **Ecosystem integration (mandatory item):** kyu is the counterparty (by definition); token injection via **latch** — the unit file runs `latch run -- desk-courier` so the token never touches disk outside latch; plain env file documented as fallback. homelab n/a (runs on the PC) | Essential | Token reaches the process via environment in tests; unit file carries the latch wrapper; fallback documented. |
