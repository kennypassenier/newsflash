...
| Mini-round M10 | 2026-08-30 | Kenny: interactive action buttons wanted (read-confirmation, freezer PC-first with timeout fallback). Chosen route: requirement filed with pipeline-v2 NOW (they own the envelope schema, frozen → their mini-round); newsflash builds on their concept; M10 stays Later meanwhile. Cross-project flag: newsflash went live inside pipeline-v2's soak window (their G3 hold never reached this session; one dogfood message + the desktop subscription flagged for their reconciliation) | docs/FEATURES.md amendment + vault requirement note + this row |
