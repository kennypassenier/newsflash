# Realization plan — hub-clients (newsflash)
...
|---|---|---|---|---|
| L0 | Walking skeleton — workspace (`courier-core` + `newsflash`), `--version`, hooks live, CI written, gates green on empty crates | [meta] | `cargo test --all` green; commit blocked without IDs; gates block a broken build | done 2026-08-29 |
...
| Phase 9 · release | 2026-08-30 | Kenny approved all four items (P1 field-test evidence (logged then as "dogfood"), P2 version 0.1.0 + changelog, P3 mechanics: tag on main HEAD, no assets by decision, P4 "tag and publish immediately"). v0.1.0 tagged and the GitHub Release published, then read back: draft=false, published 2026-08-29T22:32Z UTC, `git describe` on main sees the tag | CHANGELOG.md + the Release + this row |
| Phase 10 · retrospective | 2026-08-30 | Kenny adopted all six lessons (R-1 peer-change verification, R-2 credential planning, R-3 deployment-wiring drills, R-4 the AFK ratification pattern, R-5 widget fallback, R-6 ecosystem why-not) and approved the diff — committed to dev-procedure as 2e0f8c0. **Project complete: phases 0-10 all gated and closed** | ~/Projects/dev-procedure @ 2e0f8c0 + this row |
| Mini-round M10 | 2026-08-30 | Kenny: interactive action buttons wanted (read-confirmation, freezer PC-first with timeout fallback). Chosen route: requirement filed with pipeline-v2 NOW (they own the envelope schema, frozen → their mini-round); newsflash builds on their concept; M10 stays Later meanwhile. Cross-project flag: newsflash went live inside pipeline-v2's soak window (their G3 hold never reached this session; one field-test message + the desktop subscription flagged for their reconciliation) | docs/FEATURES.md amendment + vault requirement note + this row |
| Mini-round: rename to newsflash | 2026-08-30 | Kenny chose **newsflash** over `desk-courier` after two creative rounds (28 Japanese/Dutch-heritage names, then 30 English names, each with a rationale, all crates.io-collision-checked). One sweep: `newsflash/` crate dir, package name, systemd unit file, all doc/source references, README amendment note; live deployment re-pointed (config/state dirs, unit, latch) in the same pass. Full suite (62 tests) + all 6 live drills green post-rename | this row + README amendment |
| Mini-round: own repo | 2026-08-30 | Kenny split newsflash out of hub-clients into its own repo (4 sub-decisions, all resolved): full git history kept (clone, not filtered rewrite); courier-core moved with it (no cross-repo sharing built for uncertain vault-courier); old hub-clients DELETED — GitHub (Kenny's own admin action, gh token lacked delete_repo scope) + local (`rm -rf`, verified identical history first). New repo public, protection on and read back, first CI green. Live cutover verified at every layer: latch secret re-bound without re-minting, binary reinstalled, systemd WorkingDirectory corrected, field-test toast rendered through the enabled unit | this row + docs/SCOPE.md S1 amendment |
| Mini-round M10 built | 2026-08-30 | pipeline-v2 approved the action-button contract (their K12) same day; session was marked BLOCKED BY that project per standing rule 22 while it worked, then Kenny relayed their approved contract and asked for the build to resume. Built AR23–AR27 (interactive render split, decoupled ack timing, the live-drill-found no-cap-for-critical rule, the action_result shape, truncation-not-poison for 3+ actions); 8 new unit tests, 1 loop-level test proving the full wire-up, and a real click on a real critical toast on Kenny's own desktop, verified end to end against the scratch hub. Full suite: 64 tests, all 6 live drills green. FEATURES.md M10 raised Later → Essential | docs/ARCHITECTURE_DECISIONS.md AR23-27 + docs/FEATURES.md + docs/DRILL_LOG.md + this row |
| Stress test — multi-toast, popup limit, icons | 2026-08-30 | Kenny stress-tested M10 (300-message flood, a 2-20 button ladder, controlled multi-toast drills). Confirmed live: multiple simultaneous action-button toasts coexist and each stay independently answerable, no cross-talk (S6e, both an automated test — `m10_multiple_critical_toasts_stay_independently_answerable` — and a live 2-critical-toast drill). Found and traced to KDE Plasma's own source three non-configurable mechanisms that can strip a non-critical toast's buttons at any point in its lifetime (a ~4-count limit, an 80%-screen-height fill cap, and a reflow-on-close effect) — accepted as an external limitation (AR28, SCOPE S6f); standing guidance is that only `critical` carries a reliable whole-lifetime button guarantee. AR11 revised: an urgency-based visual-differentiation attempt (`info`→`low`) was built, live-tested, found to do nothing (Plasma has no urgency styling), and reverted the same session in favour of a per-priority `--icon`, which does work — live-confirmed through the rebuilt binary. Two requirements filed with pipeline-v2: per-message duration override (D1) and a richer default action set backed by the button-ladder measurement (D3) | docs/ARCHITECTURE_DECISIONS.md AR28 + AR11 revision + docs/SCOPE.md S6e/S6f + docs/DRILL_LOG.md + Obsidian vault requirement doc + this row |
